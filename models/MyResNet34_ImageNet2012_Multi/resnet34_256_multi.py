...
class Single_model:
    def __init__(self, optimizer, schduler, device="cuda") -> None:
        self.file_name = f"MyResNet34_{BATCH}_{optimizer}_{SCHDULER}"
        self.optim_name = optimizer
        """define model"""
        self.model = MyResNet34(num_classes=COUNT_OF_CLASSES, num_layer_factor=5).to(
            device
        )
        ...
        elif optimizer == "NAdam_decoupled_weight_decay":
            self.optimizer = torch.optim.NAdam(
                self.model.parameters(), weight_decay=1e-4, decoupled_weight_decay=True
            )
